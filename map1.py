...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # --- ⚡ OPTIMIZATION: Fully Vectorized Pivot and Calculation ---
    # 1. Create pivots for both timestamp types. This is fast and vectorized.
    ...
    # 2. Iterate through the small STAGING_LIMITS dictionary (this loop is not a performance bottleneck)
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        # 3. Select the correct timestamp series from the pre-computed pivots
        ...
        # Check if the required steps exist in the pivot tables
        if start_step not in start_pivot.columns or end_step not in end_pivot.columns:
            continue

        start_ts = start_pivot[start_step]
        end_ts = end_pivot[end_step]

        # 4. Combine the series into a temporary DataFrame for this check
        df_check = pd.concat([start_ts, end_ts], axis=1, keys=['start', 'end']).dropna()

        if df_check.empty:
            continue

        # 5. Perform fully vectorized calculations
        time_diff = (df_check['end'] - df_check['start']).dt.total_seconds() / 3600
        ...
        # 6. Create a DataFrame of results for this check and append
        records_df = pd.DataFrame({
            'Lot ID': df_check.index,
            'Staging Check': description,
            'Actual Time (hrs)': time_diff,
            'Limit (hrs)': limit_hours,
            'Start Step TS': df_check['start'].dt.strftime('%Y-%m-%d %H:%M') + f" ({start_ts_type.replace('TRACK_', '').replace('_TS', '')})",
            'End Step TS': df_check['end'].dt.strftime('%Y-%m-%d %H:%M') + f" ({end_ts_type.replace('TRACK_', '').replace('_TS', '')})",
            'Violation': time_diff > limit_hours
        })
        all_staging_records.append(records_df)

    if not all_staging_records:
        return pd.DataFrame()

    # 7. Concatenate all results at the end
