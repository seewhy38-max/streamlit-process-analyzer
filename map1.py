...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            ...
            # ⚡ OPTIMIZATION: Vectorized approach to replace iterrows()
            # This avoids a slow, row-by-row loop by building a new DataFrame segment
            # from array operations, which is significantly faster in pandas.
            report_segment = pd.DataFrame({
            ...
            all_staging_records.append(report_segment)
    ...
    # Final sorting and column selection remains the same
