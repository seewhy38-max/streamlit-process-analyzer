...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # This replaces the slow, iterative row-by-row processing with a fast, vectorized approach.
    # By operating on entire DataFrame columns at once, this is significantly more performant.
    all_staging_dfs = []
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        df_check = staging_pivot[['ASSEMBLY_LOT', start_step, end_step]].dropna().copy()
        ...
        # Assign calculated and constant values (vectorized)
        df_check['Actual Time (hrs)'] = time_diff_hours[valid_mask]
        ...
        # Format timestamps using vectorized string operations
        start_ts_type_str = f" ({config['start_ts'].replace('TRACK_', '').replace('_TS', '')})"
        end_ts_type_str = f" ({config['end_ts'].replace('TRACK_', '').replace('_TS', '')})"
        df_check['Start Step TS'] = df_check[start_step].dt.strftime('%Y-%m-%d %H:%M') + start_ts_type_str
        df_check['End Step TS'] = df_check[end_step].dt.strftime('%Y-%m-%d %H:%M') + end_ts_type_str

        # Rename Lot ID column for consistency
        df_check.rename(columns={'ASSEMBLY_LOT': 'Lot ID'}, inplace=True)

        all_staging_dfs.append(df_check)

    if not all_staging_dfs:
        return pd.DataFrame() # Return empty if no staging checks were possible

    # Concatenate all results and format the final report
