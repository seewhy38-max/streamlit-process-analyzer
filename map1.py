...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            ...
            # Vectorized approach to create report records
            ...
            report_for_step = pd.DataFrame({
                ...
                'Start Step TS': df_check[start_col].dt.strftime('%Y-%m-%d %H:%M') + start_ts_suffix,
                'End Step TS': df_check[end_col].dt.strftime('%Y-%m-%d %H:%M') + end_ts_suffix,
