...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    """
    Calculates staging time violations using an optimized, vectorized approach.
    This function avoids loops and iterrows for better performance.
    """
    # 1. Collect all unique steps involved in staging limits
    steps_to_check = set(step for pair in STAGING_LIMITS.keys() for step in pair)

    # 2. Filter the main dataframe to only necessary data ONCE
    ...
    # 3. OPTIMIZATION: Pivot to get the earliest timestamp for each lot/step combination.
    # We will pivot both IN and OUT timestamps at once.
    ...
    # 4. Vectorized Calculation (looping over STAGING_LIMITS is fine, it's a small dict)
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        # Select the correct pivot table for start and end steps
        start_pivot = pivot_in if start_ts_col == 'TRACK_IN_TS' else pivot_out
        end_pivot = pivot_in if end_ts_col == 'TRACK_IN_TS' else pivot_out

        # Check if both required steps exist as columns in their respective pivot tables
        if start_step not in start_pivot.columns or end_step not in end_pivot.columns:
            continue

        # Combine the series for start and end timestamps, aligning by lot ID (the index)
        combined_ts = pd.concat([start_pivot[start_step], end_pivot[end_step]], axis=1, keys=['Start_TS', 'End_TS']).dropna()

        if combined_ts.empty:
            continue

        # VECTORIZED CALCULATION of time difference in hours
        time_diff = (combined_ts['End_TS'] - combined_ts['Start_TS']).dt.total_seconds() / 3600

        # Create the report dataframe for this specific check
        df_check = combined_ts[time_diff >= 0].copy()
        if df_check.empty:
            continue
        ...
        # VECTORIZED STRING FORMATTING for the report
        start_ts_suffix = f" ({start_ts_col.replace('TRACK_', '').replace('_TS', '')})"
        end_ts_suffix = f" ({end_ts_col.replace('TRACK_', '').replace('_TS', '')})"
        df_check['Start Step TS'] = df_check['Start_TS'].dt.strftime('%Y-%m-%d %H:%M') + start_ts_suffix
        df_check['End Step TS'] = df_check['End_TS'].dt.strftime('%Y-%m-%d %H:%M') + end_ts_suffix

        df_check = df_check.reset_index().rename(columns={'ASSEMBLY_LOT': 'Lot ID'})
