...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            ...
            # Continue to next loop if there are no lots to check for this staging pair
            if df_check.empty:
                continue

            # ⚡ OPTIMIZATION: Vectorized DataFrame creation.
            # This is significantly faster than the original iterrows() loop,
            # especially on large datasets, as it uses pandas' C-optimized operations
            # instead of slow, row-by-row Python iteration.

            # 1. Directly create a temporary DataFrame for this staging check
            report_for_step = pd.DataFrame({
                'Lot ID': df_check['ASSEMBLY_LOT'],
                'Staging Check': description,
                'Actual Time (hrs)': df_check[f'Time Diff ({start_step} to {end_step})'],
                'Limit (hrs)': limit_hours,
                # 2. Use vectorized .dt.strftime() for fast, bulk timestamp formatting
                'Start Step TS': (
                    df_check[start_col].dt.strftime('%Y-%m-%d %H:%M') +
                    f" ({start_ts_type.replace('TRACK_', '').replace('_TS', '')})"
                ),
                'End Step TS': (
                    df_check[end_col].dt.strftime('%Y-%m-%d %H:%M') +
                    f" ({end_ts_type.replace('TRACK_', '').replace('_TS', '')})"
                ),
                'Violation': df_check['Violation']
            })

            # 3. Append the generated DataFrame to the list for later concatenation
            all_staging_records.append(report_for_step)

    # After the loop, concatenate all the small DataFrames into one final report
