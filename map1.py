...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    """
    ⚡ BOLT OPTIMIZATION:
    This function was refactored to eliminate slow, iterative pandas operations.
    - Replaced a manual for-loop pivot with a more efficient pd.melt and pd.pivot_table.
    - Removed the .iterrows() loop entirely, replacing it with vectorized calculations for time differences,
      violation checks, and string formatting.
    This results in a significant performance improvement, especially with large datasets.
    """
    # 1. Determine all unique steps and required timestamps from the configuration
    steps_to_check = set()
    required_ts_map = {}
    for (start, end), config in STAGING_LIMITS.items():
        steps_to_check.add(start)
        steps_to_check.add(end)
        required_ts_map[start] = config['start_ts']
        required_ts_map[end] = config['end_ts']

    # 2. Filter the initial data to only the relevant lots and steps
    ...
    # 3. ⚡ OPTIMIZATION: Vectorized Pivot Preparation
    # Instead of looping, we "melt" the data to get a single timestamp column,
    # then map the correct timestamp (IN or OUT) for each step.
    ...
    # Map the required timestamp type for each step
    df_melted['Required_TS'] = df_melted['SPECNAME'].map(required_ts_map)

    # Filter only the rows where the timestamp type matches the required one for that step
    df_pivot_ready = df_melted[df_melted['Timestamp_Type'] == df_melted['Required_TS']].copy()

    # 4. ⚡ OPTIMIZATION: Use pivot_table for high-performance reshaping
    # Get the earliest timestamp for each lot at each step
    staging_pivot = df_pivot_ready.pivot_table(
        index='ASSEMBLY_LOT',
        columns='SPECNAME',
        values='Timestamp',
        aggfunc='min' # Use min to handle cases where a lot passes a step multiple times
    )

    # 5. ⚡ OPTIMIZATION: Vectorized Calculation Loop (replaces iterrows)
    all_staging_records = []
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        # Check if both steps exist in the pivot table columns
        if start_step in staging_pivot.columns and end_step in staging_pivot.columns:
            ...
            df_check = df_check.reset_index() # Move ASSEMBLY_LOT from index to column
            ...
            # Filter out negative durations (end time before start time)
            df_check = df_check[df_check['Actual Time (hrs)'] >= 0].copy()

            if df_check.empty:
                continue

            # Vectorized violation check
            ...
            df_check['Start Step TS'] = df_check[start_step].dt.strftime('%Y-%m-%d %H:%M') + start_suffix
            df_check['End Step TS'] = df_check[end_step].dt.strftime('%Y-%m-%d %H:%M') + end_suffix

            # Add constant values for this check
            df_check['Staging Check'] = description
            df_check['Limit (hrs)'] = limit_hours
            df_check.rename(columns={'ASSEMBLY_LOT': 'Lot ID'}, inplace=True)

            # Append the fully formed dataframe to our list
            all_staging_records.append(df_check)
