...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    - Replaced manual pivot creation with a more efficient `pivot_table` approach.
    - Eliminated the primary performance bottleneck by removing the `.iterrows()` loop
      and using vectorized calculations for the final report.
    """
    # 1. Determine all unique steps involved in staging limits and their required timestamp types.
    steps_to_check = set()
    ts_info = {}
    for (start, end), config in STAGING_LIMITS.items():
        steps_to_check.add(start)
        steps_to_check.add(end)
        ts_info[start] = config['start_ts']
        ts_info[end] = config['end_ts']

    # 2. Filter the main dataframe to only the necessary rows and columns.
    df_staging_base = df_filtered[
        (df_filtered['ASSEMBLY_LOT'].isin(all_affected_lots_base)) &
        (df_filtered['SPECNAME'].isin(steps_to_check))
    ][['ASSEMBLY_LOT', 'SPECNAME', 'TRACK_IN_TS', 'TRACK_OUT_TS']].dropna()

    if df_staging_base.empty:
        return pd.DataFrame()

    # 3. Melt the dataframe to make timestamps a single column, which is easier to pivot.
    df_melted = df_staging_base.melt(
        id_vars=['ASSEMBLY_LOT', 'SPECNAME'],
        value_vars=['TRACK_IN_TS', 'TRACK_OUT_TS'],
        var_name='TS_TYPE',
        value_name='Timestamp'
    )

    # 4. Keep only the rows that match the required timestamp type for each step (e.g., TRACK_IN for start).
    df_melted['REQUIRED_TS'] = df_melted['SPECNAME'].map(ts_info)
    df_relevant_ts = df_melted[df_melted['TS_TYPE'] == df_melted['REQUIRED_TS']].copy()

    # 5. Create the pivot table. This is much faster than the manual loop.
    # We take the 'min' timestamp to handle cases where a lot might pass through a step multiple times.
    pivot = df_relevant_ts.pivot_table(
        index='ASSEMBLY_LOT',
        columns='SPECNAME',
        values='Timestamp',
        aggfunc='min'
    ...
    # 6. Vectorized report generation.
    all_results = []
    for (start_step, end_step), config in STAGING_LIMITS.items():
        # Check if the pivot table contains the necessary steps for this check.
        if start_step not in pivot.columns or end_step not in pivot.columns:
            continue

        # Create a working dataframe for this specific staging rule.
        ...
        # Perform all calculations for this rule in a vectorized manner.
        time_diff = (df_check[end_step] - df_check[start_step]).dt.total_seconds() / 3600.0

        # Only keep records with a non-negative time difference.
        valid_records = time_diff >= 0
        if not valid_records.any():
            continue

        df_result = pd.DataFrame({
            'Lot ID': df_check.loc[valid_records, 'ASSEMBLY_LOT'],
            'Staging Check': config['description'],
            'Actual Time (hrs)': time_diff[valid_records],
            'Limit (hrs)': config['limit_hours'],
            'Start Step TS': df_check.loc[valid_records, start_step].dt.strftime('%Y-%m-%d %H:%M') + f" ({config['start_ts'].replace('TRACK_', '').replace('_TS', '')})",
            'End Step TS': df_check.loc[valid_records, end_step].dt.strftime('%Y-%m-%d %H:%M') + f" ({config['end_ts'].replace('TRACK_', '').replace('_TS', '')})",
            'Violation': time_diff[valid_records] > config['limit_hours']
        })
    ...
    # 7. Concatenate, sort, and return the final report.
    df_report = pd.concat(all_results, ignore_index=True)
    df_report = df_report.sort_values(['Staging Check', 'Actual Time (hrs)'], ascending=[True, False]).reset_index(drop=True)

    return df_report[['Lot ID', 'Staging Check', 'Actual Time (hrs)', 'Limit (hrs)', 'Start Step TS', 'End Step TS', 'Violation']]
