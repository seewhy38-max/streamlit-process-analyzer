...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # --- Vectorized Pivot Table Creation (Replaces the slow Python loop) ---

    # 1. Melt the DataFrame to long format to handle different timestamp columns (TRACK_IN_TS vs TRACK_OUT_TS)
    ...
    # 2. Filter only the rows that match the required timestamp type for each step
    # Create a mapping Series from `required_pivot_cols` for efficient filtering
    step_to_ts_type_map = pd.Series(required_pivot_cols)
    df_melted['REQUIRED_TS_TYPE'] = df_melted['SPECNAME'].map(step_to_ts_type_map)

    # Keep rows where the TIMESTAMP_TYPE matches the REQUIRED_TS_TYPE for that step
    df_filtered_melt = df_melted[df_melted['TIMESTAMP_TYPE'] == df_melted['REQUIRED_TS_TYPE']].copy()

    # 3. Create the pivot table
    # We group by lot and step, and take the minimum timestamp (as the original logic did)
    staging_pivot = df_filtered_melt.pivot_table(
        index='ASSEMBLY_LOT',
        columns='SPECNAME',
        values='TIMESTAMP',
        aggfunc='min'  # Use 'min' to replicate the original logic of taking the earliest timestamp
    )
    all_staging_records = []
