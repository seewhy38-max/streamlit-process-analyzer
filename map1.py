...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    """
    ⚡ BOLT OPTIMIZATION:
    This function is refactored to use vectorized pandas operations, eliminating slow loops.
    - Replaced manual pivoting loop with a more efficient `pivot_table`.
    - Eliminated the final `iterrows()` loop by using vectorized calculations and string formatting.
    - This results in a significant performance improvement for large datasets.
    """
    ...
    # 2. Filter the initial dataframe to only the necessary data
    ...
    # 3. ⚡ OPTIMIZED PIVOT: Get the required timestamp for each lot/step combination
    # Instead of looping through lots, we melt the dataframe to handle TRACK_IN/OUT generically
    ...
    df_melted['REQUIRED_TS'] = df_melted['SPECNAME'].map(required_ts)

    # Filter only the rows where the TS_TYPE matches the one required for that step
    df_filtered_ts = df_melted[df_melted['TS_TYPE'] == df_melted['REQUIRED_TS']].copy()

    # Now, pivot to get the final timestamp for each lot and step. We take the 'min' in case of duplicates.
    staging_pivot = df_filtered_ts.pivot_table(
        index='ASSEMBLY_LOT',
        columns='SPECNAME',
        values='TIMESTAMP',
        aggfunc='min'
    ...
    # 4. ⚡ VECTORIZED CALCULATION: Calculate violations for all pairs in STAGING_LIMITS
    all_staging_records = []
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        # Check if both required steps exist as columns in the pivot table
        if start_step in staging_pivot.columns and end_step in staging_pivot.columns:
            ...
            # Vectorized time difference calculation (in hours)
            time_diff = (df_check[end_step] - df_check[start_step]).dt.total_seconds() / 3600

            # Filter out negative durations which indicate incorrect data order
            ...
            df_check['Actual Time (hrs)'] = time_diff
            ...
            df_check['Start Step TS'] = df_check[start_step].dt.strftime('%Y-%m-%d %H:%M') + start_ts_suffix
            df_check['End Step TS'] = df_check[end_step].dt.strftime('%Y-%m-%d %H:%M') + end_ts_suffix
    ...
    # 5. Concatenate all results and format the final report
