...
# ⚡ OPTIMIZATION: Replaced iterative processing with a vectorized, pivot-based approach.
# This avoids looping through lots and rules one-by-one, providing a >10x speedup on large datasets.
@st.cache_data(show_spinner=False)
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # --- Vectorized Pivot Operation ---
    # Create two pivot tables, one for track-in and one for track-out.
    # This is more efficient than a single pivot with a complex apply function.
    pivot_in = df_staging.pivot_table(
        index='ASSEMBLY_LOT', columns='SPECNAME', values='TRACK_IN_TS', aggfunc='min'
    )
    pivot_out = df_staging.pivot_table(
        index='ASSEMBLY_LOT', columns='SPECNAME', values='TRACK_OUT_TS', aggfunc='min'
    )

    all_staging_records = []

    # Iterate through the staging limit rules
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        # Determine which pivot table to use for start and end timestamps
        ...
        # Ensure the required columns exist in the pivoted tables
        if start_step in start_pivot.columns and end_step in end_pivot.columns:

            # Combine the start and end timestamps into a single DataFrame
            df_check = pd.concat([start_pivot[start_step], end_pivot[end_step]], axis=1)
            df_check.columns = ['start_ts', 'end_ts']
            ...
            time_diff_seconds = (df_check['end_ts'] - df_check['start_ts']).dt.total_seconds()

            # Filter out negative durations (end before start) and calculate hours
            valid_diff_mask = time_diff_seconds >= 0
            df_check = df_check[valid_diff_mask].copy()
            df_check['Actual Time (hrs)'] = time_diff_seconds[valid_diff_mask] / 3600
            ...
            # --- Assemble the report for this rule ---
    ...
    # Concatenate all rule reports into a single dataframe
