...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # OPTIMIZATION: Use pivot_table for faster reshaping instead of manual loops.
    # This avoids iterating over each lot and performing slow filtering inside the loop.

    # 1. Separate steps based on which timestamp is needed (Track In vs. Track Out)
    start_ts_steps = [step for step, ts_col in required_pivot_cols.items() if ts_col == 'TRACK_IN_TS']
    end_ts_steps = [step for step, ts_col in required_pivot_cols.items() if ts_col == 'TRACK_OUT_TS']

    # 2. Create two separate pivot tables, one for each timestamp type
    pivot_in = df_staging[df_staging['SPECNAME'].isin(start_ts_steps)].pivot_table(
        index='ASSEMBLY_LOT',
        columns='SPECNAME',
        values='TRACK_IN_TS',
        aggfunc='min'
    ...
    pivot_out = df_staging[df_staging['SPECNAME'].isin(end_ts_steps)].pivot_table(
        index='ASSEMBLY_LOT',
        columns='SPECNAME',
        values='TRACK_OUT_TS',
        aggfunc='min'
    ...
    # 3. Concatenate the results and reset the index to make ASSEMBLY_LOT a column
