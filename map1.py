...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            ...
            # OPTIMIZATION: Vectorized creation of records instead of looping with iterrows()
            # This is significantly faster on large datasets.
            df_check['Staging Check'] = description
            df_check['Limit (hrs)'] = limit_hours

            # Create formatted timestamp strings using vectorized operations
            ...
            # Rename columns to match the final report structure
            ...
            required_cols = ['Lot ID', 'Staging Check', 'Actual Time (hrs)', 'Limit (hrs)', 'Start Step TS', 'End Step TS', 'Violation']
            all_staging_records.extend(df_check[required_cols].to_dict('records'))
