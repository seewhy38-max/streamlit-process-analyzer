...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # ⚡ OPTIMIZATION: Replaced manual lot-by-lot looping with vectorized pivot_table operations.
    # This avoids iterating through unique lots, which is significantly faster on large datasets.

    # 1. Create two pivot tables, one for TRACK_IN and one for TRACK_OUT timestamps.
    ...
    # 2. Build the combined pivot table by selecting the correct timestamp (IN or OUT) for each step.
    staging_pivot = pd.DataFrame(index=pivot_in.index)
    required_pivot_cols = {}
    ...
    # ⚡ OPTIMIZATION: Replaced the second loop and iterrows() with fully vectorized calculations.
    # All staging checks are now computed in parallel for all lots, avoiding row-by-row operations.
    all_records_dfs = []
    ...
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        start_col, end_col = start_step, end_step

        # Ensure both required step columns exist in our pivot table
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:

            # 3. Perform vectorized calculation for one staging check at a time
            df_check = staging_pivot[[start_col, end_col]].dropna().copy()

            if df_check.empty:
                continue

            # Calculate time difference in hours for all lots at once
            ...
            # Determine violations, format timestamps, and add metadata - all vectorized
            ...
            # Append the formatted result for this staging check to our list of dataframes
            all_records_dfs.append(df_check)

    if not all_records_dfs:
        return pd.DataFrame()

    # 4. Concatenate all results into a single report DataFrame
    df_report = pd.concat(all_records_dfs, ignore_index=True)
