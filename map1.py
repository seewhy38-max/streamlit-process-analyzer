...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            ...
            # 2. Add other necessary columns for the report
            df_check['Staging Check'] = description
            df_check['Limit (hrs)'] = limit_hours

            # 3. Select and rename columns to build the report segment
            report_segment = df_check[[
                'ASSEMBLY_LOT',
                'Staging Check',
                time_diff_col,
                'Limit (hrs)',
            ...
            # 4. Append the processed DataFrame segment to the list
    ...
    # Concatenate all segments at once for efficiency
