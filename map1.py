...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    # This function is optimized to avoid iterrows(). It processes all staging checks
    # using vectorized pandas operations for significantly better performance on large datasets.

    # 1. Determine all unique steps involved in staging limits to pre-filter the data
    steps_to_check = set()
    ...
    # 2. Ensure timestamps are valid datetime objects
    df_staging.dropna(subset=['TRACK_IN_TS', 'TRACK_OUT_TS'], how='any', inplace=True)

    all_staging_records = []

    # 3. Loop through each staging rule (this loop is small and fixed, not over data rows)
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        # 4. VECTORIZED: Get the earliest start and end timestamps for each lot at the respective steps
        start_times = df_staging[df_staging['SPECNAME'] == start_step].groupby('ASSEMBLY_LOT')[start_ts_col].min().reset_index()
        end_times = df_staging[df_staging['SPECNAME'] == end_step].groupby('ASSEMBLY_LOT')[end_ts_col].min().reset_index()

        # 5. MERGE: Combine start and end times for each lot
        merged_df = pd.merge(
            start_times.rename(columns={start_ts_col: 'start_ts'}),
            end_times.rename(columns={end_ts_col: 'end_ts'}),
            on='ASSEMBLY_LOT',
            how='inner'
        )
        ...
        # 6. VECTORIZED CALCULATION: Compute time differences for all lots at once
        merged_df['Actual Time (hrs)'] = (merged_df['end_ts'] - merged_df['start_ts']).dt.total_seconds() / 3600

        # Filter out invalid negative time differences (data quality check)
        merged_df = merged_df[merged_df['Actual Time (hrs)'] >= 0].copy()

        # 7. VECTORIZED COMPARISON: Check for violations for all lots at once
        ...
        # 8. VECTORIZED STRING FORMATTING: Prepare timestamp strings for display
        start_ts_type = start_ts_col.replace('TRACK_', '').replace('_TS', '')
        end_ts_type = end_ts_col.replace('TRACK_', '').replace('_TS', '')
        merged_df['Start Step TS'] = merged_df['start_ts'].dt.strftime('%Y-%m-%d %H:%M') + f" ({start_ts_type})"
        merged_df['End Step TS'] = merged_df['end_ts'].dt.strftime('%Y-%m-%d %H:%M') + f" ({end_ts_type})"

        # 9. Finalize columns for this staging check
        merged_df.rename(columns={'ASSEMBLY_LOT': 'Lot ID'}, inplace=True)
        all_staging_records.append(merged_df[['Lot ID', 'Staging Check', 'Actual Time (hrs)', 'Limit (hrs)', 'Start Step TS', 'End Step TS', 'Violation']])

    # 10. Concatenate results from all staging checks
    ...
    if not df_report.empty:
        ...
        # Final column selection to match the original output format
