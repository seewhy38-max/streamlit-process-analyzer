...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # --- OPTIMIZATION: Replace manual pivot loop with vectorized pivot_table ---
    track_in_steps = {step for step, ts_col in required_pivot_cols.items() if ts_col == 'TRACK_IN_TS'}
    track_out_steps = {step for step, ts_col in required_pivot_cols.items() if ts_col == 'TRACK_OUT_TS'}

    # Create two separate pivot tables for track-in and track-out steps
    pivot_in = df_staging[df_staging['SPECNAME'].isin(track_in_steps)].pivot_table(
        index='ASSEMBLY_LOT', columns='SPECNAME', values='TRACK_IN_TS', aggfunc='min'
    )
    pivot_out = df_staging[df_staging['SPECNAME'].isin(track_out_steps)].pivot_table(
        index='ASSEMBLY_LOT', columns='SPECNAME', values='TRACK_OUT_TS', aggfunc='min'
    )

    # Combine them. Lots with data in only one will get NaN in the other, which is correct.
    staging_pivot = pd.concat([pivot_in, pivot_out], axis=1).reset_index()
    all_staging_records = []

    # --- OPTIMIZATION: Vectorize violation calculation, eliminating iterrows() ---
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            ...
            temp_df = staging_pivot[['ASSEMBLY_LOT', start_col, end_col]].dropna().copy()

            time_diff_col = f'Time Diff ({start_step} to {end_step})'
            temp_df[time_diff_col] = (temp_df[end_col] - temp_df[start_col]).dt.total_seconds() / 3600

            temp_df = temp_df[temp_df[time_diff_col] >= 0].copy()

            if temp_df.empty:
                continue

            temp_df['Violation'] = temp_df[time_diff_col] > limit_hours
            ...
            report_cols = ['Lot ID', 'Staging Check', 'Actual Time (hrs)', 'Limit (hrs)', 'Start Step TS', 'End Step TS', 'Violation']
            all_staging_records.extend(temp_df[report_cols].to_dict('records'))
