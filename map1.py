...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            ...
            # Avoids the slow iterrows() loop by creating columns directly
            if not df_check.empty:
                # 1. Create timestamp strings in a vectorized way
                ...
                # 2. Add static data for this staging check
                ...
                # 4. Append the processed DataFrame to our list
                all_staging_records.append(df_check)

    # If the list contains DataFrames, concatenate them; otherwise, create an empty DataFrame
    if all_staging_records:
        df_report = pd.concat(all_staging_records, ignore_index=True)
    else:
        df_report = pd.DataFrame(columns=['Lot ID', 'Staging Check', 'Actual Time (hrs)', 'Limit (hrs)', 'Start Step TS', 'End Step TS', 'Violation'])
