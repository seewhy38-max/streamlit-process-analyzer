...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # OPTIMIZATION: Replaced iterative pivot creation with vectorized pandas operations.
    # 1. Melt the dataframe to convert TRACK_IN_TS and TRACK_OUT_TS into rows.
    df_melt = df_staging.melt(
        id_vars=['ASSEMBLY_LOT', 'SPECNAME'],
        value_vars=['TRACK_IN_TS', 'TRACK_OUT_TS'],
        var_name='TS_Col_Name',
        value_name='Timestamp'
    )

    # 2. Create a mapping series from the required pivot columns dictionary.
    required_ts_series = pd.Series(required_pivot_cols)

    # 3. Add a new column to the melted dataframe that specifies which timestamp is needed for each step.
    df_melt['Required_TS_Col'] = df_melt['SPECNAME'].map(required_ts_series)

    # 4. Filter the melted dataframe to keep only the rows where the timestamp type matches the requirement.
    df_melt_filtered = df_melt[df_melt['TS_Col_Name'] == df_melt['Required_TS_Col']].copy()

    # 5. Pivot the filtered data to create the final staging_pivot table.
    staging_pivot = df_melt_filtered.pivot_table(
        index='ASSEMBLY_LOT',
        columns='SPECNAME',
        values='Timestamp',
        aggfunc='min'  # Use min() to match original logic for duplicate entries
    ).reset_index()
    all_staging_records = []
    ...
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            ...
            # OPTIMIZATION: Replaced iterrows with vectorized operations for report generation.
            if not df_check.empty:
                # 1. Create formatted timestamp strings directly on the DataFrame.
                ...
                # 2. Add other required report columns.
                df_check['Staging Check'] = description
                df_check['Limit (hrs)'] = limit_hours

                # 3. Rename columns to match the final report format.
                df_report_segment = df_check.rename(columns={
                    'ASSEMBLY_LOT': 'Lot ID',
                    f'Time Diff ({start_step} to {end_step})': 'Actual Time (hrs)'
                })

                # 4. Select and append the required columns.
                all_staging_records.append(df_report_segment[[
                    'Lot ID', 'Staging Check', 'Actual Time (hrs)', 'Limit (hrs)',
                    'Start Step TS', 'End Step TS', 'Violation'
                ]])

    df_report = pd.concat(all_staging_records, ignore_index=True) if all_staging_records else pd.DataFrame()
