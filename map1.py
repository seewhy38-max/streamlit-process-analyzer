...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            ...
            # --- Vectorized Operation (Replaces iterrows) ---
            # ⚡ OPTIMIZATION: Replaced slow iterrows() loop with vectorized pandas operations.
            # This avoids iterating row-by-row and applies transformations to the entire series at once.
            # Expected Impact: Significant speedup, especially on large datasets.
            if not df_check.empty:
                # 1. Create formatted timestamp strings vectorially
