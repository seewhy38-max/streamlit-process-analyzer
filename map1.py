...
# --- Staging Time Calculation (PERFORMANCE OPTIMIZED) ---
@st.cache_data(show_spinner=False)
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    """
    Calculates staging time violations using an efficient, vectorized approach.
    - Replaces slow loops with pd.melt and pivot_table for high-performance data reshaping.
    - Eliminates .iterrows() in favor of vectorized calculations for speed.
    """
    # 1. SETUP: Determine all unique steps involved and filter the initial DataFrame.
    steps_to_check = {step for key in STAGING_LIMITS.keys() for step in key}
    ...
    # 2. EFFICIENT PIVOT: Reshape data to get the earliest timestamp for each lot/step combination.
    # This replaces the slow, manual, per-lot loop.
    df_melted = df_staging.melt(
        id_vars=['ASSEMBLY_LOT', 'SPECNAME'],
        value_vars=['TRACK_IN_TS', 'TRACK_OUT_TS'],
        var_name='Timestamp_Type',
        value_name='Timestamp'
    )

    # Get the earliest relevant timestamp for each lot-step-type combination.
    pivot_base = df_melted.groupby(['ASSEMBLY_LOT', 'SPECNAME', 'Timestamp_Type'])['Timestamp'].min().reset_index()

    # Create a unique column for pivoting that combines step and timestamp type
    pivot_base['Step_TS_Type'] = pivot_base['SPECNAME'] + '_' + pivot_base['Timestamp_Type']

    # The final pivot, creating a wide table with one row per lot for fast lookups.
    staging_pivot = pivot_base.pivot_table(
        index='ASSEMBLY_LOT',
        columns='Step_TS_Type',
        values='Timestamp',
        aggfunc='first'
    )

    # 3. VECTORIZED CALCULATION: Loop through the (small) STAGING_LIMITS dictionary
    # and perform calculations on the entire pivot table at once.
    all_staging_records = []
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        # Define the column names in the pivot table (e.g., '4282_TRACK_IN_TS')
        start_col = f"{start_step}_{start_ts_type}"
        end_col = f"{end_step}_{end_ts_type}"

        # Ensure both required columns exist after the pivot
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            # Create a temporary DataFrame for this specific check, dropping lots that didn't go through both steps
            df_check = staging_pivot[[start_col, end_col]].dropna().copy()

            if df_check.empty:
                continue

            # Vectorized calculation of time difference in hours
            time_diff_hours = (df_check[end_col] - df_check[start_col]).dt.total_seconds() / 3600

            # Filter out negative durations (end before start), which are invalid data points
            df_check = df_check[time_diff_hours >= 0].copy()
            df_check['Actual Time (hrs)'] = time_diff_hours[time_diff_hours >= 0]

            if df_check.empty:
                continue

            # Vectorized creation of new columns for the report
            df_check['Lot ID'] = df_check.index
            df_check['Staging Check'] = description
            df_check['Limit (hrs)'] = limit_hours
            df_check['Violation'] = df_check['Actual Time (hrs)'] > limit_hours

            # Vectorized string formatting for timestamps for cleaner display
    ...
    # 4. FINAL ASSEMBLY: Concatenate results if any records were generated
