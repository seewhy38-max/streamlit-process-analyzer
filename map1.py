...
# --- Staging Time Calculation (⚡ OPTIMIZED) ---
@st.cache_data(show_spinner=False)
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    """
    ⚡ BOLT OPTIMIZATION:
    - Replaced slow iterative pivot and row-by-row processing with a vectorized approach.
    - Uses a single `pivot_table` to reshape the data efficiently.
    - Calculates time differences and violations using vectorized pandas operations instead of `iterrows()`.
    - Expected Performance Impact: 10-100x faster on large datasets by avoiding Python-level loops.
    """
    # 1. Prepare data by filtering for relevant lots and steps
    ...
    ].dropna(subset=['TRACK_IN_TS', 'TRACK_OUT_TS'], how='any')

    if df_staging.empty:
        return pd.DataFrame()

    # 2. OPTIMIZATION: Create a single pivot table with multi-level columns.
    # This gets the earliest TRACK_IN and TRACK_OUT time for each lot at each step in one shot.
    ...
    # 3. Iterate over the (small) STAGING_LIMITS dictionary
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        # 4. OPTIMIZATION: Use multi-level column access to get timestamp series directly
        start_col = (start_ts_type, start_step)
        end_col = (end_ts_type, end_step)

        # Check if the required columns exist in the pivot table
        if start_col in pivot_df.columns and end_col in pivot_df.columns:

            # Create a temporary df for this specific check, dropping lots that didn't go through both steps
            temp_df = pivot_df[[start_col, end_col]].dropna().copy()

            if temp_df.empty:
                continue

            # 5. OPTIMIZATION: Vectorized calculations for time difference, violation, and string formatting
            time_diff_hours = (temp_df[end_col] - temp_df[start_col]).dt.total_seconds() / 3600

            # Filter out negative durations which indicate data issues
            temp_df = temp_df[time_diff_hours >= 0].copy()
            if temp_df.empty:
                continue

            temp_df['Actual Time (hrs)'] = time_diff_hours
            temp_df['Violation'] = temp_df['Actual Time (hrs)'] > limit_hours
            ...
            temp_df['Start Step TS'] = temp_df[start_col].dt.strftime('%Y-%m-%d %H:%M') + start_ts_suffix
            temp_df['End Step TS'] = temp_df[end_col].dt.strftime('%Y-%m-%d %H:%M') + end_ts_suffix

            # Add metadata for the report
            temp_df['Staging Check'] = description
            temp_df['Limit (hrs)'] = limit_hours
            temp_df.reset_index(inplace=True) # Move ASSEMBLY_LOT from index to column
            temp_df.rename(columns={'ASSEMBLY_LOT': 'Lot ID'}, inplace=True)

            all_staging_records.append(temp_df[['Lot ID', 'Staging Check', 'Actual Time (hrs)', 'Limit (hrs)', 'Start Step TS', 'End Step TS', 'Violation']])
