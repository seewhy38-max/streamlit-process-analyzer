...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    """
    Calculates staging time violations using a vectorized approach for performance.
    - Replaces loops with pandas pivot_table, merge, and vectorized operations.
    - Expected performance gain: >10x faster for large datasets.
    """
    # 1. Prepare staging data and identify necessary steps
    steps_to_check = {step for key in STAGING_LIMITS for step in key}
    df_staging = df_filtered[
        (df_filtered['ASSEMBLY_LOT'].isin(all_affected_lots_base)) &
        (df_filtered['SPECNAME'].isin(steps_to_check))
    ].copy()
    df_staging.dropna(subset=['TRACK_IN_TS', 'TRACK_OUT_TS'], inplace=True)

    if df_staging.empty:
        return pd.DataFrame()

    # 2. Vectorized Pivot: Get the earliest timestamp for each lot/step combination
    # This replaces the first slow loop (`for lot in ...`)
    pivot_df = df_staging.pivot_table(
        index='ASSEMBLY_LOT',
        columns='SPECNAME',
        values=['TRACK_IN_TS', 'TRACK_OUT_TS'],
        aggfunc='min' # Get the first time a lot entered a step
    )

    # 3. Create a DataFrame from the STAGING_LIMITS configuration
    # This is key to enabling the vectorized merge operation
    limits_data = []
    ...
    # 4. Vectorized Calculation: Combine pivot data and limits
    # This replaces the second and third slow loops
    all_records = []
    for _, limit_row in limits_df.iterrows():
        start_step, end_step = limit_row['start_step'], limit_row['end_step']
        start_ts_col = (limit_row['start_ts_type'], start_step)
        end_ts_col = (limit_row['end_ts_type'], end_step)

        # Check if the required columns exist in the pivot table
        if start_ts_col not in pivot_df.columns or end_ts_col not in pivot_df.columns:
            continue

        # Select lots that have timestamps for both start and end steps
        relevant_lots = pivot_df.dropna(subset=[start_ts_col, end_ts_col]).copy()
        if relevant_lots.empty:
            continue

        # Extract the series for start and end timestamps
        start_ts = relevant_lots[start_ts_col]
        end_ts = relevant_lots[end_ts_col]

        # Calculate the time difference in hours (vectorized)
        time_diff_hours = (end_ts - start_ts).dt.total_seconds() / 3600

        # Filter out negative durations (e.g., data errors)
        valid_diffs = time_diff_hours[time_diff_hours >= 0]
        if valid_diffs.empty:
            continue

        # Create a temporary DataFrame for this specific staging check
        temp_df = pd.DataFrame({
            'Lot ID': valid_diffs.index,
            'Actual Time (hrs)': valid_diffs,
            'Start Step TS_raw': start_ts.loc[valid_diffs.index],
            'End Step TS_raw': end_ts.loc[valid_diffs.index]
        })

        # Add static limit information
        temp_df['Staging Check'] = limit_row['description']
        temp_df['Limit (hrs)'] = limit_row['limit_hours']

        # Determine violations (vectorized)
        temp_df['Violation'] = temp_df['Actual Time (hrs)'] > limit_row['limit_hours']

        # Format timestamp strings (vectorized)
        start_suffix = f" ({limit_row['start_ts_type'].replace('TRACK_', '').replace('_TS', '')})"
        end_suffix = f" ({limit_row['end_ts_type'].replace('TRACK_', '').replace('_TS', '')})"
        temp_df['Start Step TS'] = temp_df['Start Step TS_raw'].dt.strftime('%Y-%m-%d %H:%M') + start_suffix
        temp_df['End Step TS'] = temp_df['End Step TS_raw'].dt.strftime('%Y-%m-%d %H:%M') + end_suffix
    ...
    df_report = pd.concat(all_records, ignore_index=True)
    df_report = df_report.sort_values(['Staging Check', 'Actual Time (hrs)'], ascending=[True, False]).reset_index(drop=True)

    # Select and order final columns
    final_cols = ['Lot ID', 'Staging Check', 'Actual Time (hrs)', 'Limit (hrs)', 'Start Step TS', 'End Step TS', 'Violation']
