...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    - Replaces loops and iterrows() with pivot_table and direct DataFrame operations.
    ...
    # --- Vectorized Pivot Table Creation ---
    # 1. Map each step to its required timestamp column (e.g., '4282' -> 'TRACK_IN_TS')
    step_to_ts_col_map = {}
    for (start, end), config in STAGING_LIMITS.items():
        step_to_ts_col_map[start] = config['start_ts']
        step_to_ts_col_map[end] = config['end_ts']

    # 2. Create a 'Timestamp' column by picking the correct timestamp for each row.
    df_staging['Timestamp'] = pd.NaT
    for step, ts_col in step_to_ts_col_map.items():
        mask = df_staging['SPECNAME'] == step
        df_staging.loc[mask, 'Timestamp'] = df_staging.loc[mask, ts_col]

    # 3. Create the pivot table, which is much faster than looping.
    ...
    # --- Vectorized Staging Calculation ---
    all_staging_records = []
    for (start_step, end_step), config in STAGING_LIMITS.items():
        if start_step in staging_pivot.columns and end_step in staging_pivot.columns:
            ...
            df_check['Actual Time (hrs)'] = (df_check[end_step] - df_check[start_step]).dt.total_seconds() / 3600
            ...
            # Assign all columns using vectorized operations, no loops needed
            df_check['Lot ID'] = df_check.index
            df_check['Staging Check'] = config['description']
            df_check['Limit (hrs)'] = config['limit_hours']
            df_check['Violation'] = df_check['Actual Time (hrs)'] > config['limit_hours']

            # Vectorized string formatting for timestamps
            start_ts_suffix = f" ({config['start_ts'].replace('TRACK_', '').replace('_TS', '')})"
            end_ts_suffix = f" ({config['end_ts'].replace('TRACK_', '').replace('_TS', '')})"
    ...
    # Sort and reorder columns as before
    df_report.sort_values(['Staging Check', 'Actual Time (hrs)'], ascending=[True, False], inplace=True)
