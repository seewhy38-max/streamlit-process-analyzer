...
# ⚡ BOLT OPTIMIZATION: Replaced inefficient loops with vectorized pandas operations.
# The original function used a manual pivot and an iterrows() loop, which are slow on large datasets.
# This version uses pivot_table() for efficient reshaping and vectorized calculations for report generation,
...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # OPTIMIZATION: Vectorize the pivot operation by creating a unified timestamp column.
    # 1. Create a mapping of which timestamp column to use for each step.
    required_pivot_cols = {}
    ...
    # 2. Create the unified 'TIMESTAMP' column. This loop is small and fixed (over ~4-6 steps).
    ...
    # 3. Use pivot_table for a fast reshape.
    staging_pivot = df_staging.pivot_table(
        index='ASSEMBLY_LOT',
        columns='SPECNAME',
        values='TIMESTAMP',
        aggfunc='min'  # Use min to get the earliest timestamp if a lot hits a step multiple times
    )

    # OPTIMIZATION: Vectorize the report generation, avoiding iterrows().
    all_staging_records = []
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        # Check if the required steps exist in the pivoted data
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:

            # Start with a clean copy of the relevant data, dropping lots missing either step
            df_check = staging_pivot[[start_col, end_col]].dropna().copy()

            # Vectorized calculation of the time difference in hours
            time_diff = (df_check[end_col] - df_check[start_col]).dt.total_seconds() / 3600
            ...
            # Add all report columns using vectorized operations
            ...
            # Prepare the final DataFrame chunk for this staging rule
            df_check.reset_index(inplace=True)
            df_check.rename(columns={'ASSEMBLY_LOT': 'Lot ID'}, inplace=True)

            # Keep only the required columns
            report_chunk = df_check[['Lot ID', 'Staging Check', 'Actual Time (hrs)', 'Limit (hrs)', 'Start Step TS', 'End Step TS', 'Violation']]
            all_staging_records.append(report_chunk)

    if not all_staging_records:
        return pd.DataFrame() # Return empty if no staging checks could be performed

    # Combine all generated report chunks
    ...
    if not df_report.empty:
        ...
        # Final column selection (already done, but good practice to ensure order)
