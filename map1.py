...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    - Replaces manual pivot loop with `pivot_table`.
    - Replaces iterrows with vectorized operations.
    """
    # 1. SETUP: Determine unique steps and required timestamp columns from STAGING_LIMITS
    steps_to_check = set()
    required_ts_cols = {}
    for (start, end), config in STAGING_LIMITS.items():
        steps_to_check.add(start)
        steps_to_check.add(end)
        required_ts_cols[start] = config['start_ts']
        required_ts_cols[end] = config['end_ts']

    # 2. FILTER: Create the base DataFrame for staging analysis
    ...
    # 3. TRANSFORM: Create a 'Timestamp' column based on the required Track In/Out for each step
    # This is the key step that allows a single pivot operation
    df_staging['Timestamp'] = df_staging.apply(
        lambda row: row[required_ts_cols[row['SPECNAME']]],
        axis=1
    )

    # 4. PIVOT: Use pandas pivot_table to get one row per lot, with steps as columns
    # This is significantly faster than the manual loop
    ...
    # 5. CALCULATE: Vectorized calculation of time differences and violations
    all_staging_records = []
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_step in staging_pivot.columns and end_step in staging_pivot.columns:
            ...
            # Vectorized creation of timestamp strings for reporting
