...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            ...
            # --- ⚡ OPTIMIZATION: Vectorized timestamp formatting ---
            # Replaced iterrows() with a vectorized approach for ~10-100x speedup on large datasets.
            # This creates the formatted string columns directly without looping row-by-row.
            if not df_check.empty:
                df_check['Staging Check'] = description
                df_check['Limit (hrs)'] = limit_hours

                # 1. Format timestamps to strings
                ...
                # 2. Rename columns to match the final report structure
                df_check.rename(columns={
                    'ASSEMBLY_LOT': 'Lot ID',
                    f'Time Diff ({start_step} to {end_step})': 'Actual Time (hrs)'
                }, inplace=True)

                # 3. Select and append the required data as a dictionary list
                report_records = df_check[[
                    'Lot ID', 'Staging Check', 'Actual Time (hrs)', 'Limit (hrs)',
                    'Start Step TS', 'End Step TS', 'Violation'
                ]].to_dict('records')

                all_staging_records.extend(report_records)
