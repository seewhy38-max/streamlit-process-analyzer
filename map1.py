...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            ...
            # ⚡ OPTIMIZATION: Vectorized DataFrame creation instead of iterrows().
            # This avoids iterating row-by-row, which is slow in pandas.
            # Instead, we build all columns at once using Series operations, which is much faster.
            ...
            report_for_step = pd.DataFrame({
                ...
                'Start Step TS': df_check[start_col].dt.strftime('%Y-%m-%d %H:%M') + f" ({start_ts_type.replace('TRACK_', '').replace('_TS', '')})",
                'End Step TS': df_check[end_col].dt.strftime('%Y-%m-%d %H:%M') + f" ({end_ts_type.replace('TRACK_', '').replace('_TS', '')})",
                'Violation': df_check['Violation']
            })
            all_staging_dfs.append(report_for_step)
