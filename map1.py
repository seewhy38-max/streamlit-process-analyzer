...
# --- Staging Time Calculation (REFACTORED FOR PERFORMANCE) ---
@st.cache_data(show_spinner=False)
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    # 1. Determine all unique steps and their required timestamps from STAGING_LIMITS
    steps_to_check = set()
    # Create a mapping from SPECNAME to the required timestamp column name ('TRACK_IN_TS' or 'TRACK_OUT_TS')
    required_ts_map = {}
    ...
    # 3. Vectorized Pivot Preparation (Melt and Filter)
    # Vectorized implementation using melt/pivot to avoid slow row-wise iteration.
    # This is a highly efficient way to select the correct timestamp for each step without using .apply()

    # Melt the dataframe to make 'TRACK_IN_TS' and 'TRACK_OUT_TS' rows instead of columns
    melted = df_staging.melt(
    ...
    # Create a column that holds the required timestamp type for each SPECNAME
    melted['Required_TS_Type'] = melted['SPECNAME'].map(required_ts_map)

    # Keep only the rows where the Timestamp_Type matches the one required for that SPECNAME
    relevant_ts = melted[melted['Timestamp_Type'] == melted['Required_TS_Type']].copy()

    # 4. Create the pivot table
    # We take the 'min' timestamp in case a lot passes through the same step multiple times.
    staging_pivot = relevant_ts.pivot_table(
        index='ASSEMBLY_LOT',
        columns='SPECNAME',
        values='TIMESTAMP',
        aggfunc='min'
    ...
    # 5. Vectorized Calculation of Violations
    all_staging_records = []
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_step not in staging_pivot.columns or end_step not in staging_pivot.columns:
            continue

        # Create a temporary DataFrame for this specific rule, dropping rows with missing data
        df_check = staging_pivot[[start_step, end_step]].dropna().copy()

        if df_check.empty:
            continue

        # Calculate time difference in hours (vectorized)
        time_diff = (df_check[end_step] - df_check[start_step]).dt.total_seconds() / 3600

        # Filter for valid, non-negative durations
        df_check = df_check[time_diff >= 0].copy()
        df_check['Actual Time (hrs)'] = time_diff[time_diff >= 0]

        if df_check.empty:
            continue

        # Determine violations and add metadata (vectorized)
        df_check['Violation'] = df_check['Actual Time (hrs)'] > limit_hours
        df_check['Limit (hrs)'] = limit_hours
        df_check['Staging Check'] = description

        # Format timestamps for the report (vectorized)
    ...
    # 6. Concatenate results and finalize the report
    ...
    # Ensure final column order is consistent with the original function
