...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # --- OPTIMIZATION: Vectorized Pivot Table Preparation ---
    # 1. Create a map of which timestamp to use for each step
    required_ts_map = {}
    for (start_step, end_step), config in STAGING_LIMITS.items():
        required_ts_map[start_step] = config['start_ts']
        required_ts_map[end_step] = config['end_ts']

    # 2. Create a 'SELECTED_TS' column using the map and vectorized np.where
    df_staging['SELECTED_TS'] = np.where(
        df_staging['SPECNAME'].map(required_ts_map) == 'TRACK_IN_TS',
        df_staging['TRACK_IN_TS'],
        df_staging['TRACK_OUT_TS']
    )

    # 3. Create the pivot table in a single, fast operation
    staging_pivot = df_staging.pivot_table(
        index='ASSEMBLY_LOT',
        columns='SPECNAME',
        values='SELECTED_TS',
        aggfunc='min'  # Use min to match original logic of finding the first timestamp
    )

    # --- OPTIMIZATION: Vectorized Report Generation ---
    all_staging_records = []
    for (start_step, end_step), config in STAGING_LIMITS.items():
        # Ensure both steps exist in the pivot table to avoid KeyErrors
        if start_step not in staging_pivot.columns or end_step not in staging_pivot.columns:
            continue

        # Create a working DataFrame for this specific check, dropping lots missing either step
        ...
        # Filter out negative durations (end time is before start time)
        valid_mask = time_diff_hrs >= 0
        if not valid_mask.any():
            continue

        df_check = df_check[valid_mask]
        time_diff_hrs = time_diff_hrs[valid_mask]

        # Build the report segment for this check in a vectorized way
        df_report_part = pd.DataFrame(index=df_check.index)
        df_report_part['Staging Check'] = config['description']
        df_report_part['Actual Time (hrs)'] = time_diff_hrs
        df_report_part['Limit (hrs)'] = config['limit_hours']

        start_ts_type = config['start_ts'].replace('TRACK_', '').replace('_TS', '')
        end_ts_type = config['end_ts'].replace('TRACK_', '').replace('_TS', '')

        df_report_part['Start Step TS'] = df_check[start_step].dt.strftime('%Y-%m-%d %H:%M') + f" ({start_ts_type})"
        df_report_part['End Step TS'] = df_check[end_step].dt.strftime('%Y-%m-%d %H:%M') + f" ({end_ts_type})"
        df_report_part['Violation'] = df_report_part['Actual Time (hrs)'] > config['limit_hours']

        all_staging_records.append(df_report_part)
