...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    # 1. SETUP: Determine all unique steps and required timestamps from STAGING_LIMITS
    steps_to_check = set()
    required_ts = {}
    ...
    # 2. FILTER: Create the base DataFrame with only relevant lots and steps
    ...
    # 3. MELT & PIVOT: Efficiently get the required timestamp for each step
    # Melt the dataframe to have a single column for timestamps
    ...
    # Filter only the rows that match the required timestamp for each step
    df_melted['REQUIRED_TS'] = df_melted['SPECNAME'].map(required_ts)
    df_relevant_ts = df_melted[df_melted['TS_TYPE'] == df_melted['REQUIRED_TS']]

    # Pivot to get one row per lot with steps as columns
    staging_pivot = df_relevant_ts.pivot_table(
        index='ASSEMBLY_LOT',
        columns='SPECNAME',
        values='Timestamp',
    ...
    # 4. CALCULATE VIOLATIONS: Use vectorized operations on the pivot table
    all_staging_records = []
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        start_ts_type_suffix = f" ({config['start_ts'].replace('TRACK_', '').replace('_TS', '')})"
        end_ts_type_suffix = f" ({config['end_ts'].replace('TRACK_', '').replace('_TS', '')})"
        df_check['Start Step TS'] = df_check[start_step].dt.strftime('%Y-%m-%d %H:%M') + start_ts_type_suffix
        df_check['End Step TS'] = df_check[end_step].dt.strftime('%Y-%m-%d %H:%M') + end_ts_type_suffix
    ...
    # 5. FINALIZE: Concatenate results and format the final report
    df_report = pd.concat(all_staging_records).reset_index()
    df_report.rename(columns={'ASSEMBLY_LOT': 'Lot ID'}, inplace=True)
