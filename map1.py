...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # --- 1. Vectorized Data Filtering and Preparation ---
    ...
    # --- 2. Efficient Pivoting with Named Aggregations ---
    # Create a mapping from step to the required timestamp column ('TRACK_IN_TS' or 'TRACK_OUT_TS')
    ts_mapping = {step: config[f"{'start' if i == 0 else 'end'}_ts"]
                  for (start, end), config in STAGING_LIMITS.items()
                  for i, step in enumerate((start, end))}

    # Prepare data for pivoting by mapping the correct timestamp to a single column
    df_staging['TIMESTAMP'] = df_staging.apply(lambda row: row[ts_mapping.get(row['SPECNAME'])], axis=1)

    # Use pivot_table to get the minimum timestamp for each lot/step combination
    ...
    # --- 3. Vectorized Staging Time Calculation and Reporting ---
    all_staging_records = []
    for (start_step, end_step), config in STAGING_LIMITS.items():
        if start_step not in pivot_df.columns or end_step not in pivot_df.columns:
            continue

        # Create a temporary DataFrame for the current staging check
        df_check = pivot_df[[start_step, end_step]].dropna().copy()
        ...
        start_suffix = f" ({config['start_ts'].replace('TRACK_', '').replace('_TS', '')})"
        end_suffix = f" ({config['end_ts'].replace('TRACK_', '').replace('_TS', '')})"
        ...
        all_staging_records.append(df_check.reset_index())
    ...
    df_report = df_report.sort_values(['Staging Check', 'Actual Time (hrs)'], ascending=[True, False])
