...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # --- ⚡ OPTIMIZATION: Vectorized Pivot Operation ---
    # Replaces a slow loop over unique lots with a single, highly efficient pivot operation.
    # This is significantly faster for large datasets.

    # 1. Create a temporary column with the correct timestamp based on the step's requirement (IN or OUT).
    # We map SPECNAME to the required timestamp column (e.g., 'TRACK_IN_TS')
    ts_col_series = df_staging['SPECNAME'].map(required_pivot_cols)

    # Use np.where for a fast, vectorized conditional selection of the timestamp.
    df_staging['required_ts'] = np.where(
        ts_col_series == 'TRACK_IN_TS',
        df_staging['TRACK_IN_TS'],
        df_staging['TRACK_OUT_TS']
    )

    # 2. Perform a single pivot_table operation to reshape the data.
    # This aggregates the minimum required timestamp for each lot/step combination.
    ...
    # This loop is fast as it iterates over the small STAGING_LIMITS dictionary, not the data.
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            ...
            # Perform calculations for the current staging check
            ...
            # Filter out negative durations (e.g., end time before start time)
            ...
            df_check['Lot ID'] = df_check.index # ASSEMBLY_LOT is the index after pivot
