...
# ======================================================================
# ROOT CAUSE SCORING ENGINE (NEW v2.0)
# ======================================================================

# Global signal weights for the overall investigation confidence score.
# These mirror the weighting scheme requested for the automatic ranking.
SIGNAL_WEIGHTS = {
    'machine':   0.40,   # Common machine concentration
    'clustering':0.20,   # Timeline clustering on the suspect tool
    'uct':       0.15,   # Cycle time (UCT) abnormality
    'operator':  0.10,   # Common operator concentration
    'vendor':    0.10,   # Substrate vendor concentration
    'staging':   0.05,   # Staging time violations
}

# Map each signal to the root-cause CATEGORY it points at, plus its weight
# *within* that category. Category score = weighted blend of its signals (0-100).
CATEGORY_SIGNALS = {
    'Machine':  [('machine', 0.40), ('clustering', 0.20)],
    'Material': [('vendor', 0.10)],
    'Operator': [('operator', 0.10)],
    'Process':  [('uct', 0.15), ('staging', 0.05)],
}


def _excess_concentration(dominant_share, n_categories):
    """
    Lift-adjusted concentration strength (0-100).
    Returns 0 when the dominant share equals what you'd expect by random chance
    (1 / n_categories) and 100 when fully concentrated. This stops a step that
    only has ONE machine from producing a false "100%" signal.
    """
    if (n_categories is None or n_categories <= 1 or dominant_share is None
            or (isinstance(dominant_share, float) and np.isnan(dominant_share))):
        return 0.0
    expected = 1.0 / n_categories
    if dominant_share <= expected:
        return 0.0
    return float(np.clip((dominant_share - expected) / (1.0 - expected) * 100.0, 0, 100))


def _confidence_label(score):
    """Returns (label, hex_color) for a 0-100 score."""
    if score >= 70:
        return "HIGH POSSIBILITY", "#c0392b"
    if score >= 40:
        return "MEDIUM POSSIBILITY", "#e67e22"
    return "LOW POSSIBILITY", "#7f8c8d"


def _dominant(series):
    """Returns (value, count, total, share) for the most common value in a Series."""
    s = series.dropna()
    s = s[~s.astype(str).str.lower().isin(['nan', 'unknown', 'unknown (column missing)', ''])]
    total = int(s.shape[0])
    if total == 0:
        return None, 0, 0, 0.0
    vc = s.value_counts()
    return vc.index[0], int(vc.iloc[0]), total, float(vc.iloc[0] / total)


def compute_root_cause_engine(df_filtered, analysis_lots, affected_quantity_map, affected_lots_edv_df):
    """
    The core scoring engine. Aggregates every analysis signal into a single
    structured result used by the Executive Dashboard.

    Returns a dict with headline facts, raw signal strengths (0-100),
    category scores (Machine / Material / Operator / Process), an overall
    confidence score, and a ranked list of recommendations.
    """
    if df_filtered is None or df_filtered.empty or not analysis_lots:
        return None

    analysis_lots = [str(l).strip() for l in analysis_lots]
    critical_step_ids = [k for k in CRITICAL_STEPS.keys() if k not in ['4280', '4300']]

    df_aff = df_filtered[df_filtered['ASSEMBLY_LOT'].isin(analysis_lots)].copy()

    # Headline counts (based on the focused analysis set)
    n_lots = len(set(analysis_lots) & set(df_filtered['ASSEMBLY_LOT'].unique())) or len(analysis_lots)
    n_units = int(sum(affected_quantity_map.get(l, 0) for l in analysis_lots))

    products = sorted([
        p for p in affected_lots_edv_df.loc[
            affected_lots_edv_df.index.intersection(pd.Index(analysis_lots))
        ]['Product Name (EDV)'].unique()
        if p and p != 'NOT FOUND IN DATA'
    ])

    # ------------------------------------------------------------------
    # SIGNAL 1 + suspect step/machine: Common machine concentration
    # ------------------------------------------------------------------
    step_rows = []
    for step in critical_step_ids:
        sub_all = df_filtered[df_filtered['SPECNAME'] == step]
        if sub_all.empty:
            continue
        n_machines = int(sub_all['EQUIPMENT_LINE_NAME'].nunique())
        sub_aff = df_aff[df_aff['SPECNAME'] == step]
        if sub_aff.empty:
            continue
        lot_machine = sub_aff.groupby('ASSEMBLY_LOT')['EQUIPMENT_LINE_NAME'].first()
        dom_machine, dom_count, total, share = _dominant(lot_machine)
        if dom_machine is None:
            continue
        excess = _excess_concentration(share, n_machines)
        step_rows.append({
            'step': step,
            'step_name': CRITICAL_STEPS.get(step, step),
            'machine': dom_machine,
            'count': dom_count,
            'total': total,
            'share': share,
    ...
    suspect = None
    if step_rows:
        # Prefer steps with >=2 affected lots; rank by excess, then raw share, then count
        candidates = [r for r in step_rows if r['total'] >= 2] or step_rows
        suspect = sorted(candidates, key=lambda r: (r['excess'], r['share'], r['total']), reverse=True)[0]

    if suspect:
        machine_strength = suspect['excess']
        common_machine = suspect['machine']
        common_machine_pct = suspect['share'] * 100.0
        suspect_step = suspect['step']
        suspect_step_name = suspect['step_name']
    else:
        machine_strength = 0.0
        common_machine = "N/A"
        common_machine_pct = 0.0
        suspect_step = None
        suspect_step_name = "N/A"

    # ------------------------------------------------------------------
    # SIGNAL 2: Timeline clustering on the suspect step + machine
    # ------------------------------------------------------------------
    clustering_strength = 0.0
    if suspect_step is not None and common_machine != "N/A":
        seq = df_filtered[
            (df_filtered['SPECNAME'] == suspect_step) &
            (df_filtered['EQUIPMENT_LINE_NAME'] == common_machine)
        ].dropna(subset=['TRACK_OUT_TS']).sort_values('TRACK_OUT_TS')
        seq_lots = seq.drop_duplicates('ASSEMBLY_LOT', keep='first')['ASSEMBLY_LOT'].tolist()
        n = len(seq_lots)
        aff_set = set(analysis_lots)
        positions = [i for i, lot in enumerate(seq_lots) if lot in aff_set]
        k = len(positions)
        if k >= 2 and n >= 3:
            pos_set = set(positions)
            with_neighbor = sum(1 for p in positions if (p - 1 in pos_set) or (p + 1 in pos_set))
            adjacency = with_neighbor / k
            density = k / n            # expected adjacency under random scatter
            if density < 1.0:
                clustering_strength = float(np.clip((adjacency - density) / (1.0 - density) * 100.0, 0, 100))

    # ------------------------------------------------------------------
    # SIGNAL 3: Operator concentration at the suspect step
    # ------------------------------------------------------------------
    operator_strength = 0.0
    common_operator = "N/A"
    common_operator_pct = 0.0
    if suspect_step is not None:
        sub_all = df_filtered[df_filtered['SPECNAME'] == suspect_step]
        n_ops = int(sub_all['OPERATOR'].nunique())
        sub_aff = df_aff[df_aff['SPECNAME'] == suspect_step]
        lot_op = sub_aff.groupby('ASSEMBLY_LOT')['OPERATOR'].first()
        dom_op, _, _, op_share = _dominant(lot_op)
        if dom_op is not None:
            common_operator = dom_op
            common_operator_pct = op_share * 100.0
            operator_strength = _excess_concentration(op_share, n_ops)

    # ------------------------------------------------------------------
    # SIGNAL 4: Substrate vendor concentration
    # ------------------------------------------------------------------
    vendor_strength = 0.0
    common_vendor = "N/A"
    common_vendor_pct = 0.0
    vp, vp_exists = generate_vendor_pareto(df_filtered, analysis_lots)
    if vp_exists and not vp.empty:
        vp_known = vp[~vp['Substrate Vendor Name'].astype(str).str.contains('unknown', case=False, na=False)].copy()
        if not vp_known.empty:
            total_v = vp_known['Affected Lot Count'].sum()
            n_vendors = int(vp_known.shape[0])
            top_row = vp_known.iloc[0]
            v_share = float(top_row['Affected Lot Count'] / total_v) if total_v else 0.0
            common_vendor = top_row['Substrate Vendor Name']
            common_vendor_pct = v_share * 100.0
            vendor_strength = _excess_concentration(v_share, n_vendors)

    # ------------------------------------------------------------------
    # SIGNAL 5: Cycle time (UCT) abnormality, pooled per step
    # ------------------------------------------------------------------
    uct_strength = 0.0
    max_uct_pct = 0.0
    max_uct_step = "N/A"
    try:
        ...
        df_ct = df_ct[df_ct['SPECNAME'].isin(critical_step_ids)]
        for step in critical_step_ids:
            step_ct = df_ct[df_ct['SPECNAME'] == step]
            base = step_ct[~step_ct['ASSEMBLY_LOT'].isin(analysis_lots)]['Unit Cycle Time (min/unit)']
            aff = step_ct[step_ct['ASSEMBLY_LOT'].isin(analysis_lots)]['Unit Cycle Time (min/unit)']
            if len(base) >= 5 and len(aff) >= 1 and base.mean() > 0:
                pct = (aff.mean() - base.mean()) / base.mean() * 100.0
                if pct > max_uct_pct:
                    max_uct_pct = pct
                    max_uct_step = CRITICAL_STEPS.get(step, step)
        uct_strength = float(np.clip(max_uct_pct * 2.0, 0, 100))  # +50% increase -> 100
    except Exception:
        pass

    # ------------------------------------------------------------------
    # SIGNAL 6: Staging time violations
    # ------------------------------------------------------------------
    staging_strength = 0.0
    staging_violation_lots = 0
    staging_checked_lots = 0
    try:
        staging_df = calculate_staging_time_violations(df_filtered, analysis_lots)
        if not staging_df.empty:
            staging_checked_lots = int(staging_df['Lot ID'].nunique())
            staging_violation_lots = int(staging_df[staging_df['Violation']]['Lot ID'].nunique())
            if staging_checked_lots > 0:
                staging_strength = staging_violation_lots / staging_checked_lots * 100.0
    except Exception:
        pass

    # ------------------------------------------------------------------
    # Shift change proximity (headline only)
    # ------------------------------------------------------------------
    shift_lots = 0
    if suspect_step is not None:
        try:
            df_ct_aff = calculate_cycle_time(df_aff)
            handoff = check_shift_handoffs(df_ct_aff, analysis_lots, suspect_step)
            shift_lots = int(handoff['Lot ID'].nunique()) if not handoff.empty else 0
        except Exception:
            shift_lots = 0

    # ------------------------------------------------------------------
    # Assemble signal strengths
    # ------------------------------------------------------------------
    signals = {
        'machine': machine_strength,
        'clustering': clustering_strength,
        'uct': uct_strength,
        'operator': operator_strength,
        'vendor': vendor_strength,
        'staging': staging_strength,
    }

    # Category scores (weighted blend within each category, renormalised 0-100)
    category_scores = {}
    for cat, sig_list in CATEGORY_SIGNALS.items():
        wsum = sum(w for _, w in sig_list)
        score = sum(signals[s] * w for s, w in sig_list) / wsum if wsum else 0.0
        category_scores[cat] = round(float(score), 1)

    # Overall investigation confidence (weighted sum of all six signals)
    overall_score = round(float(sum(signals[s] * w for s, w in SIGNAL_WEIGHTS.items())), 1)

    # Signal contribution table (transparency)
    signal_label = {
        'machine': 'Common machine', 'clustering': 'Timeline clustering',
        'uct': 'Cycle time abnormality', 'operator': 'Same operator',
        'vendor': 'Vendor', 'staging': 'Staging violation',
    }
    signal_table = []
    for key, w in SIGNAL_WEIGHTS.items():
        signal_table.append({
            'Signal': signal_label[key],
            'Strength (0-100)': round(signals[key], 1),
            'Weight': w,
            'Weighted Contribution': round(signals[key] * w, 1),
        })
    ...
    # ------------------------------------------------------------------
    recs = []
    if top_cat == 'Machine' and common_machine != "N/A":
        recs.append(f"Investigate **{common_machine}** first (suspect step: {suspect_step_name})")
    elif top_cat == 'Material' and common_vendor != "N/A":
        recs.append(f"Check **{common_vendor}** substrate lots first")
    elif top_cat == 'Operator' and common_operator != "N/A":
        recs.append(f"Review handling by operator **{common_operator}** first")
    elif top_cat == 'Process':
        recs.append("Review process window / staging time control first")

    # Secondary supporting actions for any other strong signal
    if common_machine != "N/A" and machine_strength >= 40 and "Investigate" not in (recs[0] if recs else ""):
        recs.append(f"Verify tooling / chase guide pin on {common_machine}")
    if clustering_strength >= 40 and common_machine != "N/A":
        recs.append(f"Inspect {common_machine} for drift / wear-in around the affected window (review PM history)")
    if operator_strength >= 40 and common_operator != "N/A":
        recs.append(f"Review handling procedure of operator {common_operator}")
    if vendor_strength >= 40 and common_vendor != "N/A":
        recs.append(f"Raise a material check on {common_vendor} substrate")
    if staging_violation_lots > 0:
        recs.append(f"Review staging time on {staging_violation_lots} violating lot(s)")
    if shift_lots > 0:
        recs.append(f"Check shift-handoff procedure ({shift_lots} lot(s) near 6:30 boundary)")
    ...
    return {
        ...
        'common_machine_pct': common_machine_pct,
        'common_operator': common_operator,
        'common_operator_pct': common_operator_pct,
        'common_vendor': common_vendor,
        'common_vendor_pct': common_vendor_pct,
        'suspect_step_name': suspect_step_name,
        'max_uct_pct': max_uct_pct,
        'max_uct_step': max_uct_step,
        'staging_violation_lots': staging_violation_lots,
        'staging_checked_lots': staging_checked_lots,
...
def _build_text_summary(eng):
    """ASCII-style summary block for copy/paste into emails or PowerPoint."""
    def bar(score):
    ...
    lines = []
    lines.append("=" * 58)
    lines.append("        PRODUCT MAPPING INVESTIGATION SUMMARY")
    lines.append("=" * 58)
    lines.append(f"Affected Lots          {eng['n_lots']}")
    lines.append(f"Affected Units         {eng['n_units']:,}")
    lines.append(f"Products               {eng['products_str']}")
    lines.append(f"Common Machine         {eng['common_machine']} ({eng['common_machine_pct']:.0f}%)")
    lines.append(f"Common Operator        {eng['common_operator']} ({eng['common_operator_pct']:.0f}%)")
    lines.append(f"Common Vendor          {eng['common_vendor']} ({eng['common_vendor_pct']:.0f}%)")
    lines.append(f"Highest Risk Process   {eng['suspect_step_name']}")
    lines.append(f"Highest UCT Increase   +{eng['max_uct_pct']:.0f}% ({eng['max_uct_step']})")
    lines.append(f"Staging Violation      {eng['staging_violation_lots']} Lot(s)")
    lines.append(f"Shift Change           {'Yes (' + str(eng['shift_lots']) + ' lots)' if eng['shift_lots'] else 'No'}")
    lines.append("")
    lines.append("Investigation Score")
    lines.append(f"Machine    {bar(cs['Machine'])} {cs['Machine']:.0f}%")
    lines.append(f"Material   {bar(cs['Material'])} {cs['Material']:.0f}%")
    lines.append(f"Operator   {bar(cs['Operator'])} {cs['Operator']:.0f}%")
    lines.append(f"Process    {bar(cs['Process'])} {cs['Process']:.0f}%")
    lines.append("")
...
# --- NEW v2.0: Executive Dashboard ---
def render_executive_dashboard(df_filtered, all_affected_lots_base, selected_lots,
                               affected_quantity_map, affected_lots_edv_df):
    st.title("📊 Executive Investigation Summary")
    st.caption("Where should I investigate first? — auto-ranked from machine, material, operator and process signals.")
    st.markdown("---")

    analysis_lots = selected_lots if selected_lots else all_affected_lots_base

    with st.spinner("Scoring root causes..."):
        eng = compute_root_cause_engine(df_filtered, analysis_lots, affected_quantity_map, affected_lots_edv_df)

    if eng is None:
        st.warning("Not enough data to build the summary. Check that the affected lots exist in the filtered dataset.")
        return

    # ---- Top recommendation banner ----
    label, color = _confidence_label(eng['top_score'])
    st.markdown(
        f"<div style='background:{color};color:white;padding:14px 18px;border-radius:10px;"
        f"font-size:1.05em;'>🎯 <b>Primary suspect: {eng['top_category'].upper()}</b> "
        f"&nbsp;—&nbsp; {eng['top_score']:.0f}% &nbsp;<b>{label}</b></div>",
    ...
    # ---- Headline facts ----
    r1 = st.columns(4)
    r1[0].metric("Affected Lots", eng['n_lots'])
    r1[1].metric("Affected Units", f"{eng['n_units']:,}")
    r1[2].metric("Products", eng['products_str'])
    r1[3].metric("Common Machine", eng['common_machine'], f"{eng['common_machine_pct']:.0f}% of lots")

    r2 = st.columns(4)
    r2[0].metric("Common Operator", eng['common_operator'], f"{eng['common_operator_pct']:.0f}%")
    r2[1].metric("Common Vendor", eng['common_vendor'], f"{eng['common_vendor_pct']:.0f}%")
    r2[2].metric("Highest UCT Increase", f"+{eng['max_uct_pct']:.0f}%", eng['max_uct_step'])
    r2[3].metric("Staging Violations", f"{eng['staging_violation_lots']} lots",
                 f"of {eng['staging_checked_lots']} checked" if eng['staging_checked_lots'] else None)

    r3 = st.columns(4)
    r3[0].metric("Highest Risk Process", eng['suspect_step_name'])
    r3[1].metric("Shift-Change Lots", f"{eng['shift_lots']} lots",
                 "near 6:30 boundary" if eng['shift_lots'] else "none")
    r3[2].metric("Overall Signal", f"{eng['overall_score']:.0f}%")

    st.markdown("---")

    # ---- Investigation score + ranking ----
    left, right = st.columns([1.1, 1])

    with left:
        st.subheader("Investigation Score")
        for cat, score in eng['ranked']:
            _, c = _confidence_label(score)
            st.markdown(_bar_html(cat, score, c), unsafe_allow_html=True)

        st.write("")
        st.subheader("Automatic Root Cause Ranking")
        rank_df = pd.DataFrame(eng['ranked'], columns=['Cause', 'Score'])
        rank_df['Confidence'] = rank_df['Score'].apply(lambda s: _confidence_label(s)[0])
        st.dataframe(
            rank_df.style.format({'Score': '{:.0f}'}).apply(
                lambda col: ['background-color: #fde7e7; font-weight:bold;'
                             if (col.name == 'Score' and v >= 70) else '' for v in col], axis=0
            ),
    ...
    with right:
        ...
        st.caption("Each raw signal (0-100) × its weight = its contribution to the overall confidence score.")
        sig_df = pd.DataFrame(eng['signal_table'])
        st.dataframe(
            sig_df.style.format({
                'Strength (0-100)': '{:.0f}',
                'Weight': '{:.0%}',
                'Weighted Contribution': '{:.1f}',
            }).bar(subset=['Strength (0-100)'], color='#5b8def', vmin=0, vmax=100),
            use_container_width=True, hide_index=True,
        )
        st.markdown(
            f"<div style='text-align:right;font-size:0.95em;'>Overall confidence "
            f"<b>{eng['overall_score']:.0f}%</b></div>", unsafe_allow_html=True
        )
    ...
    if eng['recommendations']:
        ...
        st.info("No strong directional signal yet — broaden the lot list or check data completeness.")

    # ---- Copy/paste summary ----
    with st.expander("📋 Copy-ready text summary (for email / PowerPoint)"):
        st.code(_build_text_summary(eng), language="text")

    st.caption("Scores use lift-adjusted concentration, so a step with only one available tool/vendor "
               "does not produce a false 100%. Drill into the Comprehensive Report for the full evidence.")
    return
...
def main():
    ...
    st.set_page_config(
        layout="wide", 
        page_title="Process & Machine Mapping Analyst (v2.1 - Tabbed report + Root Cause Ranking)",
    ...
    if df is not None:
        ...
        if 'lot_table_data' not in st.session_state:
            st.session_state.lot_table_data = pd.DataFrame({
                'Lot ID': [''],
                'Affected Quantity': [1]
            })
        ...
        lot_table_edited = lot_exp.data_editor(
            ...
            num_rows="dynamic",
            use_container_width=True,
            key="sidebar_lot_table",
            column_config={
                "Lot ID": st.column_config.TextColumn(
                    "Lot ID",
                    help="Enter the Full Lot ID (e.g. 8MAA30036.04)",
                    width="medium",
                ),
                "Affected Quantity": st.column_config.NumberColumn(
                    "Qty",
                    help="Number of affected units",
                    min_value=1,
                    step=1,
                    width="small",
                    default=1,
        ...
        st.session_state.lot_table_data = lot_table_edited

        # Parse Lot Input from the table
        affected_quantity_map = {}
        for _, row in lot_table_edited.iterrows():
            raw_id = str(row.get('Lot ID', '')).strip().split('(')[0].strip()
            if not raw_id or raw_id.lower() == 'nan':
                continue
            try:
                quantity = int(row.get('Affected Quantity', 1))
                if quantity > 0:
                    affected_quantity_map[raw_id] = quantity
