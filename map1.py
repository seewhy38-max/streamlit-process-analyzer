...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # --- OPTIMIZATION: Vectorized pivot table creation ---
    # Melt the dataframe to have a single timestamp column with values from either TRACK_IN_TS or TRACK_OUT_TS
    ...
    # Filter only the rows where the ts_type matches what's required for that SPECNAME
    # Create a mapping from specname to required ts_type
    spec_to_ts_type = {step: ts_col for step, ts_col in required_pivot_cols.items()}
    df_melted['required_ts_type'] = df_melted['SPECNAME'].map(spec_to_ts_type)

    # Keep rows where the timestamp type is the one we need
    df_timestamps = df_melted[df_melted['ts_type'] == df_melted['required_ts_type']].copy()

    # Now we can pivot
    staging_pivot = df_timestamps.pivot_table(
        index='ASSEMBLY_LOT',
        columns='SPECNAME',
        values='timestamp',
        aggfunc='min'
    ...
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            ...
            df_check[time_diff_col] = (df_check[end_col] - df_check[start_col]).dt.total_seconds() / 3600

            # Filter out negative time differences
            df_check = df_check[df_check[time_diff_col] >= 0].copy()

            if df_check.empty:
                continue

            # --- OPTIMIZATION: Vectorized violation calculation and string formatting ---
            ...
            all_staging_records.append(records)
