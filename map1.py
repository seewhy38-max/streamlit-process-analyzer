...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # --- OPTIMIZATION 1: Vectorized Pivot ---
    # Create a mapping of which timestamp to use for each step
    required_pivot_cols = {}
    ...
    # Melt the dataframe to combine TRACK_IN_TS and TRACK_OUT_TS into a single column
    ...
    # Create a mapping dataframe to filter for the correct timestamp type for each step
    pivot_map_df = pd.DataFrame(list(required_pivot_cols.items()), columns=['SPECNAME', 'ts_type'])

    # Join the melted data with the mapping to select the correct timestamp for each lot/step combination
    df_mapped = pd.merge(df_melted, pivot_map_df, on=['SPECNAME', 'ts_type'])

    # Create the pivot table to get the earliest timestamp for each lot at each required step
    staging_pivot = df_mapped.pivot_table(
        index='ASSEMBLY_LOT',
        columns='SPECNAME',
        values='timestamp',
        aggfunc='min' # Use min to replicate original logic of taking the first timestamp
    )

    # --- OPTIMIZATION 2: Vectorized Report Generation ---
    all_staging_records = []
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        # Ensure both start and end steps exist in the pivot table for a valid comparison
        if start_step in staging_pivot.columns and end_step in staging_pivot.columns:

            # Select relevant columns and drop rows with missing timestamps
            df_check = staging_pivot[[start_step, end_step]].dropna().copy()

            # Vectorized calculation of time difference in hours
            time_diff_series = (df_check[end_step] - df_check[start_step]).dt.total_seconds() / 3600

            # Filter out negative or zero durations which are invalid
            valid_indices = time_diff_series >= 0
            df_check = df_check[valid_indices]
            time_diff_series = time_diff_series[valid_indices]

            if df_check.empty:
                continue

            # Vectorized creation of the report columns
            report_part = pd.DataFrame({
                'Lot ID': df_check.index,
                'Staging Check': description,
                'Actual Time (hrs)': time_diff_series,
                'Limit (hrs)': limit_hours,
                'Start Step TS': df_check[start_step].dt.strftime('%Y-%m-%d %H:%M') + f" ({start_ts_type.replace('TRACK_', '').replace('_TS', '')})",
                'End Step TS': df_check[end_step].dt.strftime('%Y-%m-%d %H:%M') + f" ({end_ts_type.replace('TRACK_', '').replace('_TS', '')})",
            })

            # Vectorized violation check
            report_part['Violation'] = report_part['Actual Time (hrs)'] > limit_hours

            all_staging_records.append(report_part)
