...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    # ⚡ OPTIMIZATION: This function was refactored to use vectorized pandas operations,
    # avoiding slow loops and iterrows() for a significant performance improvement.

    # 1. Determine all unique steps involved in staging limits and filter the data once.
    steps_to_check = {step for start, end in STAGING_LIMITS.keys() for step in (start, end)}
    ...
    # 2. Iterate through each staging rule and process it as a separate, vectorized operation.
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        # 3. Get the minimum timestamp for the start and end steps for each lot.
        df_start = df_staging[df_staging['SPECNAME'] == start_step]
        start_ts = df_start.groupby('ASSEMBLY_LOT')[start_ts_type].min().rename('Start_TS')

        df_end = df_staging[df_staging['SPECNAME'] == end_step]
        end_ts = df_end.groupby('ASSEMBLY_LOT')[end_ts_type].min().rename('End_TS')

        # 4. Merge the start and end timestamps into a single DataFrame.
        df_check = pd.merge(start_ts, end_ts, on='ASSEMBLY_LOT', how='inner')

        if df_check.empty:
            continue

        # 5. Perform vectorized calculations for time difference and violations.
        df_check['Actual Time (hrs)'] = (df_check['End_TS'] - df_check['Start_TS']).dt.total_seconds() / 3600

        # Filter out negative durations which can occur from data errors.
        ...
        # 6. Add descriptive columns for the final report.
    ...
    # 7. Concatenate all results and format the final report.
