...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    ...
    # ⚡ Bolt: Replaced iterative pivot creation with a more efficient `pivot_table` approach.
    # Instead of looping through lots and steps, we create two pivots and combine them.
    # This is significantly faster for large datasets.

    # Create one pivot for Track In times and one for Track Out times
    ...
    # Combine the pivots based on the required timestamp for each step
    staging_pivot = pd.DataFrame(index=pivot_in.index.union(pivot_out.index))
    required_pivot_cols = {}
    ...
    for step, ts_col in required_pivot_cols.items():
        if ts_col == 'TRACK_IN_TS' and step in pivot_in.columns:
            staging_pivot[step] = pivot_in[step]
        elif ts_col == 'TRACK_OUT_TS' and step in pivot_out.columns:
    ...
    for (start_step, end_step), config in STAGING_LIMITS.items():
        ...
        if start_col in staging_pivot.columns and end_col in staging_pivot.columns:
            ...
            # Create a working copy for this staging check
            df_check = staging_pivot[[start_col, end_col]].dropna().copy()

            # Calculate time difference
            df_check['Time Diff'] = (df_check[end_col] - df_check[start_col]).dt.total_seconds() / 3600

            # Filter out negative durations (incorrect data order)
            df_check = df_check[df_check['Time Diff'] >= 0].copy()

            if df_check.empty:
                continue

            # ⚡ Bolt: Vectorized the report creation to avoid slow iterrows().
            # This is significantly faster and more idiomatic for pandas.
            report = pd.DataFrame()
            report['Lot ID'] = df_check.index
            report['Staging Check'] = description
            report['Actual Time (hrs)'] = df_check['Time Diff']
            ...
            report['Start Step TS'] = df_check[start_col].dt.strftime('%Y-%m-%d %H:%M') + start_ts_suffix
            report['End Step TS'] = df_check[end_col].dt.strftime('%Y-%m-%d %H:%M') + end_ts_suffix

            report['Violation'] = df_check['Time Diff'] > limit_hours

            all_staging_records.append(report)
