...
def calculate_staging_time_violations(df_filtered, all_affected_lots_base):
    # ⚡ OPTIMIZATION: This entire function is refactored to use vectorized pandas operations,
    # eliminating slow loops (`.iterrows()`) and manual pivoting. This significantly improves performance
    # on large datasets by leveraging pandas' internal C optimizations.
    # The new approach uses `groupby` to get the first/last timestamp for each step and `merge`
    # to combine start and end steps for efficient, parallel calculation.
    ...
    # 2. Vectorized timestamp extraction for all steps at once
    # For each lot and step, get the earliest Track In and latest Track Out
    timestamps = df_staging.groupby(['ASSEMBLY_LOT', 'SPECNAME']).agg(
        TRACK_IN_TS=('TRACK_IN_TS', 'min'),
        TRACK_OUT_TS=('TRACK_OUT_TS', 'max')
    ).reset_index()

    all_staging_records = []

    # 3. Loop through staging configurations and perform merges
    for (start_step, end_step), config in STAGING_LIMITS.items():
        limit_hours = config['limit_hours']
        description = config['description']
        start_ts_col = config['start_ts']
        end_ts_col = config['end_ts']

        # Filter timestamps for the start and end steps
        start_df = timestamps[timestamps['SPECNAME'] == start_step][['ASSEMBLY_LOT', start_ts_col]].rename(columns={start_ts_col: 'Start TS'})
        end_df = timestamps[timestamps['SPECNAME'] == end_step][['ASSEMBLY_LOT', end_ts_col]].rename(columns={end_ts_col: 'End TS'})

        # Merge start and end dataframes on the lot ID
        merged_df = pd.merge(start_df, end_df, on='ASSEMBLY_LOT')

        if merged_df.empty:
            continue

        # 4. Vectorized calculation of time difference and violations
        merged_df['Actual Time (hrs)'] = (merged_df['End TS'] - merged_df['Start TS']).dt.total_seconds() / 3600
        merged_df = merged_df[merged_df['Actual Time (hrs)'] >= 0]
        merged_df['Violation'] = merged_df['Actual Time (hrs)'] > limit_hours

        # 5. Format final report columns
        merged_df['Staging Check'] = description
        merged_df['Limit (hrs)'] = limit_hours
        merged_df['Lot ID'] = merged_df['ASSEMBLY_LOT']

        # Format timestamp strings for display
        start_suffix = f" ({start_ts_col.replace('TRACK_', '').replace('_TS', '')})"
        end_suffix = f" ({end_ts_col.replace('TRACK_', '').replace('_TS', '')})"
        merged_df['Start Step TS'] = merged_df['Start TS'].dt.strftime('%Y-%m-%d %H:%M') + start_suffix
        merged_df['End Step TS'] = merged_df['End TS'].dt.strftime('%Y-%m-%d %H:%M') + end_suffix

        all_staging_records.append(merged_df[['Lot ID', 'Staging Check', 'Actual Time (hrs)', 'Limit (hrs)', 'Start Step TS', 'End Step TS', 'Violation']])

    if not all_staging_records:
        return pd.DataFrame()

    # 6. Concatenate and sort final results
